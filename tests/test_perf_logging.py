...
from cc_dump.io.perf_logging import (
    ComplexityTracker,
    monitor_complexity,
    monitor_slow_path,
)
...
def test_monitor_slow_path_no_log_below_threshold(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="cc_dump.test.perf"), monitor_slow_path(
        "test.stage",
        logger=logger,
...
def test_monitor_complexity_no_log_below_thresholds(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="cc_dump.test.cx"), monitor_complexity(
        "test.stage",
        logger=logger,
        total_items=100,
        threshold_ms=10_000.0,

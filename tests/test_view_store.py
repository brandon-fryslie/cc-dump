...
class TestSchema:
    def test_schema_has_43_keys(self):
        # 6 categories × 3 axes + 5 panel/follow + 10 footer + 6 side-channel + 4 search
        assert len(cc_dump.app.view_store.SCHEMA) == 43

    def test_schema_keys_from_category_config(self):
        for _, name, _, _ in CATEGORY_CONFIG:
            assert f"vis:{name}" in cc_dump.app.view_store.SCHEMA
            assert f"full:{name}" in cc_dump.app.view_store.SCHEMA
            assert f"exp:{name}" in cc_dump.app.view_store.SCHEMA

    def test_schema_defaults_match_category_config(self):
        for _, name, _, default in CATEGORY_CONFIG:
            assert cc_dump.app.view_store.SCHEMA[f"vis:{name}"] == default.visible
            assert cc_dump.app.view_store.SCHEMA[f"full:{name}"] == default.full
            assert cc_dump.app.view_store.SCHEMA[f"exp:{name}"] == default.expanded
...
class TestReconcile:
    def test_reconcile_preserves_values(self):
        ...
        store.reconcile(cc_dump.app.view_store.SCHEMA, lambda s: [])
...
class TestPanelAndFollowSchema:
    ...
    def test_follow_default(self):
        assert cc_dump.app.view_store.SCHEMA["nav:follow"] == "active"

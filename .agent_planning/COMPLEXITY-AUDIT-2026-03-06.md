...
### 5. Multi-Provider Proxy
**Complexity: HIGH | Value: MODERATE | Verdict: SIMPLIFY**
- **What:** Secondary provider endpoints (OpenAI-compatible) with per-provider proxy servers
- **Where:** `providers.py` (166 lines), `cli.py` (60 lines), touches 7+ files
- **Files touched:** 7 (cli.py, proxy.py, har_recorder.py, har_replayer.py, sessions.py, lifecycle_controller.py, app.py)
- **State:** `proxy_servers`, `proxy_handlers`, `proxy_ports`, `proxy_targets`, `provider_endpoints` — all per-provider dicts
- **Special-case logic:** 6 branches in cli.py alone for provider iteration
- **Why it's complex:** The `provider_endpoints` dict is weakly-typed `dict[str, dict[str, str | object]]` flowing through 7+ layers. Each consumer reaches in by string key. The CA cert path key is only conditionally present.
- **Boundary erosion:** Pervasive. `providers.py` is imported by 7+ modules. Hardcoded `"anthropic"` strings in cli.py (lines 429, 601, 618) instead of using `DEFAULT_PROVIDER_KEY`.
- **If simplified:** Replace the untyped dict with a proper `ProviderEndpoint` dataclass. Use `DEFAULT_PROVIDER_KEY` consistently.
...
### 9. System Prompt Tracking with Diffs
**Complexity: MODERATE | Value: HIGH | Verdict: KEEP**
- **What:** Color-coded diff annotations for system prompt changes between API calls
- **Where:** `formatting_impl.py` (`track_content()`, `make_diff_lines()`), `rendering_impl.py` (6 renderer variants)
- **State:** `FormatRuntime.positions` dict — cross-turn mutable state tracking seen content
- **Coupling:** Accumulates state in `FormatRuntime` — hidden statefulness behind functional API.
...
### 11. Filterset Presets (F1–F9)
**Complexity: LOW | Value: MODERATE | Verdict: KEEP**
- **What:** Function keys apply named visibility presets; = / - cycle through them
- **Where:** `settings.py` (DEFAULT_FILTERSETS), `action_handlers.py`, `action_config.py`
- **Dead code:** `get_filterset()` reads saved filtersets from settings.json, validates them, warns if stale, then ignores them entirely. ~5 lines of dead validation machinery.

### 12. Save Filterset Preset (Shift+F1–F9)
**Complexity: N/A | Value: N/A | Verdict: FEATURE DOES NOT EXIST**
- **What:** This feature is listed in documentation but has no implementation. There is no save path for user-defined filtersets. `get_filterset()` always returns built-in defaults.
- **Dead code:** The stale-data check in `get_filterset()` is the only remnant.

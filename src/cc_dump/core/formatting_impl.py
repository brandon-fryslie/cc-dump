...
class ProviderRuntimeState:
    ...
    // (the caller boundary), not inside format_request.
    """
...
def populate_content_regions(block: "FormattedBlock") -> None:
    ...
    for i, sb in enumerate(seg.sub_blocks):
        ...
        tags: list[str] = []
        if sb.kind == cc_dump.core.segmentation.SubBlockKind.XML_BLOCK:
            tags = [sb.meta.tag_name]
        elif sb.kind == cc_dump.core.segmentation.SubBlockKind.CODE_FENCE and sb.meta.info:
            tags = [sb.meta.info]
...
class FormattedBlock:
    ...
    _segment_result: cc_dump.core.segmentation.SegmentResult | None = field(
        default=None,
        repr=False,
        compare=False,
    )
...
def _format_user_text_content(text: str, ctx: _ContentContext) -> list[FormattedBlock]:
    ...
    for sb in seg.sub_blocks:
        ...
        if sb.kind == cc_dump.core.segmentation.SubBlockKind.XML_BLOCK:
            meta = sb.meta
            if not isinstance(meta, cc_dump.core.segmentation.XmlBlockMeta):
...
# ─── Provider dispatch ────────────────────────────────────────────────────────
# // [LAW:dataflow-not-control-flow] Dispatch table keyed by protocol family.
...
def format_request_for_provider(provider: str, body, state: ProviderRuntimeState, request_headers=None, cache_zones=None) -> list:
    ...
    # [LAW:dataflow-not-control-flow] Anthropic is the only provider family, so
    #   request formatting is unconditional — no per-family branch.

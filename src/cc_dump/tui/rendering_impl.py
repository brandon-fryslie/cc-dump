...
class RenderRuntime:
    ...
    // [LAW:one-source-of-truth] Theme-dependent runtime state lives in this object.
    """
...
# // [LAW:one-source-of-truth] Default runtime instance for module-callers that
# // do not pass an explicit runtime through public boundaries.
...
def create_render_runtime() -> RenderRuntime:
    """Create a fresh render runtime with no theme configured."""
...
def set_default_render_runtime(runtime: RenderRuntime) -> RenderRuntime:
    """Replace default render runtime and return previous runtime.

    // [LAW:single-enforcer] Runtime replacement flows through one boundary.
    """
...
def reset_render_runtime_for_tests() -> RenderRuntime:
    """Reset default render runtime to an uninitialized state for tests.

    Returns previous runtime so callers can restore it.
    """
    previous = _default_render_runtime
...
def use_render_runtime(runtime: RenderRuntime | None):
    """Temporarily bind a runtime for nested rendering calls."""
    if runtime is None:
        yield
        return
...
def _role_styles() -> dict[str, str]:
    return _active_runtime().role_styles


def _tag_styles() -> list[tuple[str, str]]:
    return _active_runtime().tag_styles


def _msg_colors() -> list[str]:
    return _active_runtime().msg_colors


def _filter_indicators() -> dict[str, tuple[str, str]]:
...
def _render_message_header(
    ...
    *,
    ...
    style = _role_styles().get(role_lower, "bold magenta")
...
def _render_region_block_strips(
    ...
    def _region_expanded(region) -> bool | None:
        ...
        if ctx.overrides is not None:
            region_vs = ctx.overrides._regions.get((block.block_id, region.index))
            if region_vs is not None:
                return region_vs.expanded
        return None

    region_cache_state = tuple(
        (region.index, _region_expanded(region)) for region in block.content_regions
    )
    cache_key = (
        block.block_id,
        ctx.render_width,
    ...
    for part_renderable, region_idx in region_parts:
        ...
        if part_lines:
            part_strips = [
                strip.adjust_cell_length(ctx.render_width)
...
def _render_standard_block_strips(
    ...
    block_strips = (
        [strip.adjust_cell_length(ctx.render_width) for strip in Strip.from_lines(lines)]
        if lines
        else []
    )
    if cache is not None:
...
def _compute_expandable(
    ...
    collapsed_key = (block_type, vis.visible, vis.full, False)
    expanded_key = (block_type, vis.visible, vis.full, True)
    has_different_expanded = RENDERERS.get(collapsed_key) is not RENDERERS.get(
        expanded_key
    )
    exceeds_collapsed_limit = (
        collapsed_limit is not None
        and collapsed_limit > 0
        and len(block_strips) > collapsed_limit
    )
    return bool(children) or has_different_expanded or exceeds_collapsed_limit


def _prepare_gutter_inputs(
    block_strips: list[Strip],
    vis: VisState,
    max_lines: int | None,
    is_expandable: bool,
    ctx: _RenderContext,
) -> tuple[list[Strip], str]:
    is_truncated = (
    ...
    if is_truncated:
        ...
        strips_for_gutter = list(block_strips[:max_lines])
        strips_for_gutter.append(_make_collapse_indicator(hidden, ctx.render_width))
        arrow = GUTTER_ARROWS.get((vis.full, False), "") if is_expandable else ""
        return strips_for_gutter, arrow

    strips_for_gutter = list(block_strips)
    arrow = GUTTER_ARROWS.get((vis.full, vis.expanded), "") if is_expandable else ""
    return strips_for_gutter, arrow


def _emit_block_with_gutter(
    block: FormattedBlock,
    block_strips: list[Strip],
    indicator_name: str | None,
    is_expandable: bool,
    ...
    if not _should_render_block(block, block_strips):
        return
    ...
    final_strips = _add_gutter_to_strips(
        ...
        arrow,
        ctx.width,
        neutral=indicator_name is None,
...
def _recurse_visible_children(
    children: list[FormattedBlock],
    vis: VisState,
    ctx: _RenderContext,
) -> None:
    # // [LAW:single-enforcer] Child recursion policy is centralized here.
...
def _render_block_tree(block: FormattedBlock, ctx: _RenderContext) -> None:
    ...
    _recurse_visible_children(children, vis, ctx)
...
def render_streaming_preview_with_runtime(
    ...
    console,
    width: int,
    runtime: RenderRuntime | None,
) -> list:
    """Runtime-aware variant of render_streaming_preview."""
    # // [LAW:one-source-of-truth] Runtime binding is explicit at this boundary.
...
def render_block(block: FormattedBlock) -> ConsoleRenderable | None:
    ...
    """
    return render_block_with_runtime(block, runtime=None)


def render_block_with_runtime(
    block: FormattedBlock,
    runtime: RenderRuntime | None,
) -> ConsoleRenderable | None:
    """Runtime-aware variant of render_block."""
    # // [LAW:one-source-of-truth] Runtime binding is explicit at this boundary.
    with use_render_runtime(runtime):

...
from surview.formatting import (
    ...
    ErrorBlock, ProxyErrorBlock, LogBlock, NewlineBlock,
    make_diff_lines,
)
...
_EXPANDABLE_BLOCK_TYPES = frozenset({"TrackedContentBlock"})
...
def _render_system_label(block: SystemLabelBlock, filters: dict) -> Text | None:
    ...
    if not filters.get("system", False):
        return None
    t = Text("SYSTEM:", style=f"bold {surview.palette.PALETTE.system}")
...
def _render_stream_tool_use(block: StreamToolUseBlock, filters: dict) -> Text | None:
    ...
    if not filters.get("tools", False):
        return None
    t = Text("\n  ")
    t.append("[tool_use]", style=f"bold {surview.palette.PALETTE.info}")
    t.append(" " + block.name)
    return _add_filter_indicator(t, "tools")
...
def _render_error(block: ErrorBlock, filters: dict) -> Text | None:
    """Render error block."""
    return Text("\n  [HTTP {} {}]".format(block.code, block.reason), style=f"bold {surview.palette.PALETTE.error}")


def _render_proxy_error(block: ProxyErrorBlock, filters: dict) -> Text | None:
    """Render proxy error block."""
    return Text("\n  [PROXY ERROR: {}]".format(block.error), style=f"bold {surview.palette.PALETTE.error}")

...
from surview.tui.custom_footer import StyledFooter

# Use module-level imports so hot-reload takes effect
import surview.formatting
import surview.tui.widget_factory
import surview.tui.event_handlers


class SurviewApp(App):
    """TUI application for surview."""
    ...
    def compose(self) -> ComposeResult:
        ...
        conv = surview.tui.widget_factory.create_conversation_view()
        conv.id = self._conv_id
        yield conv

        economics = surview.tui.widget_factory.create_economics_panel()
        economics.id = self._economics_id
        yield economics

        timeline = surview.tui.widget_factory.create_timeline_panel()
        timeline.id = self._timeline_id
        yield timeline

        logs = surview.tui.widget_factory.create_logs_panel()
        logs.id = self._logs_id
        yield logs

        stats = surview.tui.widget_factory.create_stats_panel()
    ...
    def on_mount(self):
        ...
        self._log("INFO", "🚀 surview proxy started")
    ...
    def _process_replay_data(self):
        ...
        for req_headers, req_body, resp_status, resp_headers, complete_message in self._replay_data:
            try:
                ...
                request_blocks = surview.formatting.format_request(req_body, self._state)

                # Add request headers if present
                if req_headers:
                    header_blocks = surview.formatting.format_request_headers(req_headers)
                    # Insert after MetadataBlock
                    for i, block in enumerate(request_blocks):
                        if isinstance(block, surview.formatting.MetadataBlock):
                ...
                if resp_headers:
                    response_blocks.extend(
                        surview.formatting.format_response_headers(resp_status, resp_headers)
                    )

                # Add complete message blocks (NO streaming events)
                response_blocks.extend(
                    surview.formatting.format_complete_response(complete_message)
                )
    ...
    async def _check_hot_reload(self):
        ...
        try:
            reloaded_modules = surview.hot_reload.check_and_get_reloaded()
        except Exception as e:
        ...
        try:
            if "surview.tui.widget_factory" in reloaded_modules:
    ...
    async def _replace_all_widgets_inner(self):
        ...
        new_conv = surview.tui.widget_factory.create_conversation_view()
        new_conv.restore_state(conv_state)

        new_stats = surview.tui.widget_factory.create_stats_panel()
        new_stats.restore_state(stats_state)

        new_economics = surview.tui.widget_factory.create_economics_panel()
        new_economics.restore_state(economics_state)

        new_timeline = surview.tui.widget_factory.create_timeline_panel()
        new_timeline.restore_state(timeline_state)

        new_logs = surview.tui.widget_factory.create_logs_panel()
    ...
    def _handle_event_inner(self, event):
        ...
        if kind == "request_headers":
            self._app_state = surview.tui.event_handlers.handle_request_headers(
                event, self._state, widgets, self._app_state, log_callback
            )

        elif kind == "request":
            self._app_state = surview.tui.event_handlers.handle_request(
                event, self._state, widgets, self._app_state, log_callback
            )

        elif kind == "response_headers":
            self._app_state = surview.tui.event_handlers.handle_response_headers(
                event, self._state, widgets, self._app_state, log_callback
            )
            ...
            self._app_state = surview.tui.event_handlers.handle_response_event(
                event, self._state, widgets, self._app_state, log_callback
            )

        elif kind == "response_done":
            self._app_state = surview.tui.event_handlers.handle_response_done(
                event, self._state, widgets, self._app_state, refresh_callbacks, db_context, log_callback
            )

        elif kind == "error":
            self._app_state = surview.tui.event_handlers.handle_error(
                event, self._state, widgets, self._app_state, log_callback
            )

        elif kind == "proxy_error":
            self._app_state = surview.tui.event_handlers.handle_proxy_error(
                event, self._state, widgets, self._app_state, log_callback
            )
    ...
    def on_unmount(self):
        ...
        self._log("INFO", "surview TUI shutting down")
        self._closing = True
        self._router.stop()

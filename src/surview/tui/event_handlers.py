...
def handle_request(event, state, widgets, app_state, log_fn):
    ...
    try:
        blocks = surview.formatting.format_request(body, state)
        ...
        if pending_headers:
            ...
            if meta_idx is not None:
                ...
                header_blocks = surview.formatting.format_request_headers(pending_headers)
...
def handle_response_headers(event, state, widgets, app_state, log_fn):
    ...
    try:
        blocks = surview.formatting.format_response_headers(status_code, headers_dict)
        if blocks:
            conv = widgets["conv"]
...
def handle_response_event(event, state, widgets, app_state, log_fn):
    ...
    try:
        blocks = surview.formatting.format_response_event(event_type, data)
        ...
        for block in blocks:
            ...
            if isinstance(block, surview.formatting.StreamInfoBlock):
...
def handle_error(event, state, widgets, app_state, log_fn):
    ...
    block = surview.formatting.ErrorBlock(code=code, reason=reason)
...
def handle_proxy_error(event, state, widgets, app_state, log_fn):
    ...
    block = surview.formatting.ProxyErrorBlock(error=err)

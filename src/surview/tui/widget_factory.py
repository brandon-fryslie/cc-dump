...
import surview.tui.rendering
import surview.tui.panel_renderers
import surview.db_queries
...
class TurnData:
    ...
    def compute_relevant_keys(self):
        ...
        for block in self.blocks:
            key = surview.tui.rendering.BLOCK_FILTER_KEY.get(type(block).__name__)
            if key is not None:
                keys.add(key)
    ...
    def re_render(
        ...
        self.strips, self.block_strip_map = surview.tui.rendering.render_turn_to_strips(
...
class ConversationView(ScrollView):
    ...
    def add_turn(self, blocks: list, filters: dict = None):
        ...
        strips, block_strip_map = surview.tui.rendering.render_turn_to_strips(
    ...
    def append_streaming_block(self, block, filters: dict = None):
        ...
        if type(block).__name__ == "TextDeltaBlock":
            ...
            # Render this block
            rendered = surview.tui.rendering.render_block(block, filters)
    ...
    def finalize_streaming_turn(self) -> list:
        ...
        from surview.formatting import TextContentBlock
        ...
        if width != self._last_width and width > 0:
            ...
            for td in self._turns:
                ...
                if td.is_streaming:
                    continue
                overrides = self._overrides_for_turn(td.turn_index)
                td.strips, td.block_strip_map = surview.tui.rendering.render_turn_to_strips(
...
class StatsPanel(Static):
    ...
    def refresh_from_db(self, db_path: str, session_id: str, current_turn: dict = None):
        ...
        stats = surview.db_queries.get_session_stats(db_path, session_id, current_turn)
    ...
    def _refresh_display(self, input_tokens: int, output_tokens: int,
        ...
        text = surview.tui.panel_renderers.render_stats_panel(
            self.request_count,
            input_tokens,
            output_tokens,
            cache_read_tokens,
...
class ToolEconomicsPanel(Static):
    ...
    def refresh_from_db(self, db_path: str, session_id: str):
        ...
        rows = surview.db_queries.get_tool_economics(db_path, session_id, group_by_model=self._breakdown_mode)
    ...
    def _refresh_display(self, rows):
        ...
        text = surview.tui.panel_renderers.render_economics_panel(rows)
        self.update(text)
...
class TimelinePanel(Static):
    ...
    def refresh_from_db(self, db_path: str, session_id: str):
        ...
        turn_data = surview.db_queries.get_turn_timeline(db_path, session_id)
        ...
        for row in turn_data:
            ...
            budget = surview.analysis.compute_turn_budget(request_body)
    ...
    def _refresh_display(self, budgets: list):
        """Rebuild the timeline table."""
        text = surview.tui.panel_renderers.render_timeline_panel(budgets)
        self.update(text)
...
class LogsPanel(RichLog):
    """Panel showing surview application logs (debug, errors, internal messages)."""

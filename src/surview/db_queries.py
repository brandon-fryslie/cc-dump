...
def get_tool_invocations(db_path: str, session_id: str) -> list:
...
def get_tool_economics(db_path: str, session_id: str, group_by_model: bool = False) -> list:
    ...
    try:
        ...
        cursor = conn.execute("""
            WITH turn_totals AS (
                SELECT
                    ti.turn_id,
                    SUM(ti.input_tokens) as turn_tool_total
                FROM tool_invocations ti
                JOIN turns t ON ti.turn_id = t.id
                WHERE t.session_id = ?
                GROUP BY ti.turn_id
            ...
            SELECT
                ...
                CAST(
                    CASE
                        WHEN tt.turn_tool_total > 0 AND t.cache_read_tokens > 0
                        THEN (CAST(ti.input_tokens AS REAL) / tt.turn_tool_total) * t.cache_read_tokens
                        ELSE 0
                    END
                AS INTEGER) as cache_read_contrib
            FROM tool_invocations ti
            JOIN turns t ON ti.turn_id = t.id
